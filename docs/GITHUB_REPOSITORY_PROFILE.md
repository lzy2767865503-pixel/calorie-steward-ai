...
原因：已选名称保留了产品语义，同时明确表达 AI 产品类别，也没有绑定某一个模型或移动平台。
...
`Camera-first, uncertainty-aware AI nutrition journal with bilingual mobile UX, deterministic scoring and managed-gateway controls. Built by LAI ZEYU (来泽宇).`

## Topics

`react-native`, `expo`, `typescript`, `nutrition`, `food-image-analysis`, `ai`, `privacy-by-design`, `sqlite`, `android`, `ios`, `digital-health`, `vibe-coding`

## Suggested release title

`Calorie Steward v1.2.3 — Improved Photo Recognition / 拍照识别优化版`

## Suggested social preview text

`One photo. Honest uncertainty. Longitudinal nutrition insight.`  
`一次拍照，诚实表达不确定性，转化为长期饮食洞察。`

## Repository settings before publication

- Public visibility, `main` as default branch.
- Enable Issues, Discussions and private vulnerability reporting.
- Enable branch protection: require the `Public integrity and tests` workflow, disallow force pushes and require conversation resolution.
...
- Use an annotated tag such as `v1.2.3`, publish the signed APK and its
